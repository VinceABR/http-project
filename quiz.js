/*
  ...
  One external JavaScript file handles the responsive navigation menu
  on every page and the self-assessment quiz on quiz.html.
*/

// Responsive menu: available on every page.
...
if (menuToggle && mainMenu) {
  ...
  // Restore desktop navigation if the browser is enlarged after mobile use.
...
// Quiz logic only runs when the quiz form exists.
const quizForm = document.getElementById("quiz-form");
const resetButton = document.getElementById("reset-button");
const resultsPanel = document.getElementById("quiz-results");

if (quizForm && resetButton && resultsPanel) {
  const answers = {
    q1: "stateless",
    q2: "GET",
    q3: "404",
  ...
  quizForm.addEventListener("submit", function (event) {
    ...
    showFeedback(5, q5Correct, q5Selected.join(", "), "TLS encryption, Content Delivery Networks, and Gzip/Brotli compression");
